...
static int find_flips(volatile uint8_t *mem, size_t mem_size, uint8_t pattern, int *to0, int *to1) {
            ...
            for ( int use_flush = 0; use_flush < 2; use_flush++ ) {
                ...
                for ( int row_idx = 0; row_idx < 60; row_idx++ ) {
                    ...
                    for ( int dir = 0; dir < 2; dir++ ) {
                        ...
                        memset((void *)aggr[0], (dir) ? 0x55 : 0xaa, 0x1000);
                        memset((void *)aggr[1], (dir) ? 0x55 : 0xaa, 0x1000);
                        memset((void *)aggr[2], (dir) ? 0x55 : 0xaa, 0x1000);
                        memset((void *)aggr[3], (dir) ? 0x55 : 0xaa, 0x1000);
                        memset((void *)victim, (dir) ? 0xaa : 0x55, 0x1000);
                        ...
                        int usable_flips = find_flips(victim, 0x1000, (dir) ? 0xaa : 0x55, &to0, &to1);
